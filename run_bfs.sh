...
[[ $degree =~ ^[1-9][0-9]*$ && $iterations =~ ^[1-9][0-9]*$ ]] ||
    die "degree and iterations must be positive integers"

[[ $root =~ ^(0|[1-9][0-9]*)$ && $type =~ ^[012]$ ]] ||
    die "Invalid root or type"

for variant in "${variants[@]}"; do
    case "$variant" in
        none|pref|advise|pref_advise) ;;
        *) die "Unknown variant: $variant" ;;
    esac
done

# Save all output from this run to a timestamped file.
result_file="bfs_results_$(date +'%Y%m%d_%H%M%S').txt"
...
for index in "${!sizes[@]}"; do
    ...
    [[ $(wc -c < "$graph.col") -eq $((16 + (nodes + 1) * 8)) &&
       $(wc -c < "$graph.dst") -eq $((16 + edges * 8)) ]] ||
        die "Incomplete graph files"
